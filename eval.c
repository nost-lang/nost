...
static nost_val eval(nost_vm* vm, nost_ctx* ctx, nost_val expr);

static nost_val call(nost_vm* vm, nost_ctx* ctx, nost_val fn, nost_val args) {
    int argLen = nost_len(vm, args);
    if(nost_symIs(fn, "var")) {
        if(argLen != 2) {
            nost_error err;
            nost_initError(&err, "Invalid # of args for var.");
            nost_rtError(ctx, err);
            return nost_nil();
        }
        nost_val nameVal = nost_nth(vm, args, 0);
        nost_val val = eval(vm, ctx, nost_nth(vm, args, 1)); 
        ...
        if(!nost_addDynvar(vm, ctx, name)) {
            nost_error err;
            nost_initError(&err, "Variable redeclaration.");
            nost_rtError(ctx, err);
            return nost_nil();
        }
        nost_setVar(ctx, name, val);
        return val;
    }
    nost_error err;
...
static nost_val eval(nost_vm* vm, nost_ctx* ctx, nost_val expr) {
    if(nost_isNil(expr)) {
        ...
        nost_optVal varVal = nost_getVar(ctx, name);
        if(varVal.nil) {
            nost_error err;
        ...
        return call(vm, ctx, fn, nost_cdr(vm, cons)); 
    }
